...
int main(void)
{
    ...
    assert(runtime.session.experience == umi_integration_studio_runtime_experience());
    assert(runtime.session.active_panel_count > 0U);
    assert(runtime.commands.command_count > 0U);
    assert(umi_integration_studio_runtime_health(NULL, NULL, &health) == UMI_STATUS_OK);
